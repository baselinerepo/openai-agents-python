#--------------------------------------------------------------------
# Usage: Azure Function App for SQLite Connector Middleware
# Description: This Azure Function App provides middleware for executing SQLite queries,
...
# - http://localhost:7071/api/sqliteconnector/query
#
# Step 4: Test the endpoints using tools like Postman or curl.
#
# - Three Main Endpoints:
#
# POST  /api/sqliteconnector/query - Execute SELECT queries
# GET   /api/sqliteconnector/schema - Get database schema information
# POST  /api/sqliteconnector/procedure - Execute stored procedures
#
# - POST /api/sqliteconnector/query with JSON body containing "query" and optional "parameters"
#
# Example Usage from GPT:
# json// Query data
# POST /api/sqliteconnector/query
# {
...
#
#
#- GET /api/sqliteconnector/schema with optional "table" query parameter
#
# // Get schema
#GET /api/sqliteconnector/schema?table=Users
#
#
# - POST /api/sqliteconnector/procedure with JSON body containing "procedureName" and optional "parameters"
#
...
# Configuration
AZURE_SQLite_CNXN_STRING = os.getenv("AZURE_SQLite_CNXN_STRING", "")
...
# POST /api/sqliteconnector/query
@app.route(route="sqliteconnector/{route}", auth_level=func.AuthLevel.FUNCTION, methods=["POST"])
def sqliteconnector_query(req: func.HttpRequest) -> func.HttpResponse:
    try:
        logging.info('Python HTTP trigger function processed sqlite query request.')
        #req.route_params['route'] = 'query'
        return main(req)
    except Exception as e:
        logging.info(f"Error processing sqlite query request: {str(e)}")
...
# GET /api/sqliteconnector/schema?table=Users
@app.route(route="sqliteconnector/schema", auth_level=func.AuthLevel.FUNCTION, methods=["GET"])
def sqliteconnector_schema(req: func.HttpRequest) -> func.HttpResponse:
    logging.info('Python HTTP trigger function processed sqlite schema request.')
    req.route_params['route'] = 'schema'
    return main(req)


# POST /api/sqliteconnector/procedure
@app.route(route="sqliteconnector/procedure", auth_level=func.AuthLevel.ANONYMOUS, methods=["POST"])
def sqliteconnector_procedure(req: func.HttpRequest) -> func.HttpResponse:
    logging.info('Python HTTP trigger function processed sqlite stored procedure request.')
...
def contains_dangerous_sqlite_keywords(query: str) -> bool:
    """Check if the SQLite query contains dangerous keywords."""
...
def get_dbConnection():
    ...
    import pyodbc
    try:
        if not AZURE_SQLite_CNXN_STRING:
            raise ValueError("AZURE_SQLite_CNXN_STRING environment variables not set.")

        cnxn = pyodbc.connect(AZURE_SQLite_CNXN_STRING)
        return cnxn
    except Exception as e:
...
def execute_sqlite_query(query: str, parameters: Dict[str, Any] = None) -> Dict[str, Any]:
...
def execute_query(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Security check
        if contains_dangerous_sqlite_keywords(query):
            return func.HttpResponse(
        ...
        # Execute query
        result = execute_sqlite_query(query, parameters)
